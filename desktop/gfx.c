...
BNX_BOOL gfxInit()
{
	...
	{
		flags = SDL_WINDOW_SHOWN;//SDL_SWSURFACE|SDL_ANYFORMAT;//?????
	}

	_Gfx.windowmain = SDL_CreateWindow("BINIAX-2", 100, 100, cGfxScreenX, cGfxScreenY, flags);

	
	if ( _Gfx.windowmain == 0 )
	{
		return BNX_FALSE;
	}
	...
	_Gfx.screen = SDL_GetWindowSurface( _Gfx.windowmain );
		...
		for ( j = 0; j < cGridY; ++j )
		{
			...
			for ( i = 0; i < cGridX; ++i )
			{
				if ( game->grid[ i ][ j ] != 0 && game->grid[ i ][ j ] < cElementLimit )
				{
					...
					SDL_BlitSurface(_Gfx.elements[pairLeft(game->grid[ i ][ j ])], NULL, _Gfx.screen, &pos);
					pos.x += cGfxNextPlusX;
					SDL_BlitSurface(_Gfx.elements[pairRight(game->grid[ i ][ j ])],	NULL, _Gfx.screen, &pos);
					pos.x = tmpx;
				}
				else
				{
					switch ( game->grid[ i ][ j ] )
					{
						case cBrickFlag :
							...
							SDL_BlitSurface(_Gfx.elements[ cGfxBrickElement ], NULL, _Gfx.screen, &pos);
							pos.x = tmpx;
							pos.y = tmpy;
							break;
						default :
...
void gfxRenderEntry( void )
{
	...
	for ( alpha = 0; alpha < cGfxMaxAlpha; alpha+= cGfxAlphaStep )
	{
		...
		SDL_SetSurfaceAlphaMod(_Gfx.logo, (Uint8) alpha);
...
void gfxUpdate()
{
	SDL_UpdateWindowSurface( _Gfx.windowmain );
...
BNX_BOOL gfxLoadImage( char *filename, SDL_Surface **img )
{
	...
	if ( temp != 0 )
	{
		*img = SDL_ConvertSurfaceFormat( temp, SDL_PIXELFORMAT_RGBA8888, 0);
...
BNX_BOOL gfxLoadResources()
{
	...
	SDL_SetSurfaceAlphaMod(_Gfx.window, cGfxWinBlending);
